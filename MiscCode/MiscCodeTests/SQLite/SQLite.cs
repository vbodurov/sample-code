...
namespace MiscCodeTests.SQLite
{
	...
	public class TableMapping
	{
		...
		Column[] _columns;
		IDictionary<string, Column> _columnByName;
		IDictionary<string, Column> _columnByPropertyName;
		public Column[] Columns
		{
			...
			private set
			{
				...
				_columnByPropertyName =
					_columns.Aggregate(
						new Dictionary<string, Column>(StringComparer.InvariantCultureIgnoreCase),
						(dict, col) =>
						{
							dict[col.PropertyName] = col;
							return dict;
						});
			}
		}
		...
		public TableMapping(Type type, CreateFlags createFlags = CreateFlags.None)
		{
			...
			{
				_isTuple = true;
				_insertColumns = new Column[0];
				_insertOrReplaceColumns = new Column[0];
				CreateTupleColumns(type);
				return;
			}
			_isTuple = false;
			...
			foreach (var p in props)
			{
				...
				if (!ignore)
				{
					cols.Add(new Column(p, null, createFlags));
				}
			}
        ...
        static bool IsTypeTuple(Type tuple)
        {
            ...
            return openType == typeof(ValueTuple<>)
                   ...
                   || openType == typeof(ValueTuple<,,,,,>)
                   || openType == typeof(ValueTuple<,,,,,,>)
                   || (openType == typeof(ValueTuple<,,,,,,,>) && IsTypeTuple(tuple.GetGenericArguments()[7]));
        }
		void CreateTupleColumns(Type type)
		{
			var cols = new List<Column>();

			foreach (var field in type.GetFields())
			{
				cols.Add(new Column(null, field));
			}
			Columns = cols.ToArray();
		}
		...
		{
			return _columnByPropertyName.TryGetValue(propertyName, out var col) ? col : null;
		}

		public Column FindColumn(string columnName)
		{
			return _columnByName.TryGetValue(columnName, out var col) ? col : null;
		}
		...
		public class Column
		{
			...
			public string PropertyName => _prop?.Name ?? _field.Name;
			...
			public Column(PropertyInfo prop, FieldInfo field, CreateFlags createFlags = CreateFlags.None)
			{
	...
	public static class Orm
	{
		...
		public static object InflateAttribute(CustomAttributeData x)
		{
			...
			var r = Activator.CreateInstance(x.AttributeType, new object[0]);
	...
	public partial class SQLiteCommand
	{
		...
		public IEnumerable<T> ExecuteDeferredQuery<T>(TableMapping map)
		{
			...
			try
			{
				...
				while (SQLite3.Step(stmt) == SQLite3.Result.Row)
				{
					...
					for (int i = 0; i < cols.Length; i++)
					{
						var currentColumn = cols[i];
						if (currentColumn == null)
							continue;
						var colType = SQLite3.ColumnType(stmt, i);
						var val = ReadCol(stmt, i, colType, currentColumn.ColumnType);
						currentColumn.SetValue(obj, val);
					}
		...
		internal static void BindParameter(Sqlite3Statement stmt, int index, object value, bool storeDateTimeAsTicks, string dateTimeStringFormat, bool storeTimeSpanAsTicks)
		{
			...
			else
			{
				...
				else if (value is Guid)
				{
					...
					SQLite3.BindBlob(stmt, index, ((Guid)value).ToByteArray(), 16, NegativePointer);
				}
		...
		object ReadCol(Sqlite3Statement stmt, int index, SQLite3.ColType type, Type clrType)
		{
			...
			else
			{
				...
				{
					return SQLite3.ColumnByteArray(stmt, index).ToDateTimeOffset();
				}
				...
				else if (clrType == typeof(Guid))
				{
					...
					return new Guid(SQLite3.ColumnByteArray(stmt, index));
				}
