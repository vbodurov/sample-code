...
{
    [TestFixture]
    public class BuildNeuralNetworkDemo
    {
        [Test]
        public void Run()
        {
...
#region data
            ...
            BuildNeuralNetwork nn = new BuildNeuralNetwork(numInput, numHidden, numOutput);
...
//            TextWriter oldOut = Console.Out;
    ...
    public class BuildNeuralNetwork
    {
        ...
        public BuildNeuralNetwork(int numInput, int numHidden, int numOutput)
        {
...
//Console.WriteLine("@"+outputs.Select(n=>n.ToString("F6")).JoinStrings(",")+"|"+tValues.Select(n=>n.ToString("F6")).JoinStrings(",")+"|"+oGrads.Select(n=>n.ToString("F6")).JoinStrings(","));
            // 2. compute hidden gradients
            for (int i = 0; i < hGrads.Length; ++i)
            {
                // derivative of tanh = (1 - y) * (1 + y)
                double derivative = (1 - hOutputs[i])*(1 + hOutputs[i]);
                //double derivative = Math.Sqrt(1 - hOutputs[i]*hOutputs[i]);
                double sum = 0.0;
